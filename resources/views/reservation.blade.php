...
<script>
        ...
        $(document).ready(function(){
            ...
            $(".rooms").on('change', '.roomQuantity', function(){
                ...
                console.log(newqty)
                $(".rooms .rates_"+id).html('<small class="small">Room Rates:</small>Rs.'+total+'/<small class="small">Night</small>');
                ...
                success: function (response) {
                    ...
                    $.each(response.checkRoom, function(k,v){
                        ...
                        $.each(response.packages, function(k,v){
                             pack += '<div class="col-md-4"><section class="custom-section"><div><input type="radio" class="package" id="control_0'+v.p_id+'" data-idpackage="'+v.r_id+'" onchange="radioChange()" name="package" value="'+v.p_price+'"><label for="control_0'+v.p_id+'" class="custom-label">'+v.p_name+'</label></div></section></div>'
                        })
                        ...
                        +v.r_name+'</h2><span class="room-name">Lorem ipsum dolor sit, amet consectetur adipisicing elit. Expedita reiciendis nemo rem id quo maxime asperiores debitis deleniti a sunt unde, aspernatur at sequi quisquam aliquid velit. Alias, autem! Modi.</span></div></div><hr><div class="row">'
                            ...
                            +maxquantity+'" data-id="'+v.r_id+'" min="0" id="r_newquantity'+v.r_id+'"><br></div><div class="col-md-4"></div><div class="col-md-4 align-middle"><span class="rates rates_'+v.r_id+'"><small class="small">Room Rates: </small>Rs.00.00/<small class="small">Night</small></span></div><input type="hidden" id="rate" class="totalratebed_'
                            +v.r_id+'" name="ratebed" value="'+v.r_price+'"><input type="hidden" id="rate" class="totalrate_'+v.r_id+'" name="rate" value="'+v.r_price+'"><input type="hidden" id="fixedrate_'+v.r_id+'" name="fixedrate" value="'+v.r_price+'"></div><div class="row rates"><div class="bedRate"></div></div><div class="row"><div class="col-md-4"></div><div class="col-md-4"></div><div class="col-md-4 mobile-padding"><input type="hidden" name="id" value="'
                            +v.r_id+'" ><input type="hidden" value="'+v.r_image+'" id="image'+v.r_id+'" name="image"><input type="hidden" name="r_name" id="r_name'+v.r_id+'" value="'
                            +v.r_name+'" ><input type="hidden" class="bedtotalrate_'+v.r_id+'" value="00.00"><input type="hidden" id="roomid" value="'+v.r_id+'"><input type="hidden" id="additionalbed'+v.r_id+'" data-roomid="'+v.r_id+'" value="'+v.r_additional_bed+'"><input type="hidden" name="packagerate" value="" id="packagerate"><input type="hidden" value="'+response.days+'" id="days'+v.r_id+'" name="days"><button type="button" onClick="addToCart('+v.r_id+')" class="btn btn-warning">Reserve</button></div></div></div></div></form>')
                    })
                },
        ...
        function radioChange(){
                ...
                var subtotal = price * newqty;
                var total = +subtotal+ + packageTotal;
                console.log(total)
                
                $(".rooms .rates_"+id).html('<small class="small">Room Rates:</small>Rs.'+total+'/<small class="small">Night</small>');
                $(".rooms .totalratebed_"+id).attr({"value":total});
                $(".rooms .totalrate_"+id).attr({"value":total});
